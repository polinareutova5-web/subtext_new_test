...
Фронтенд ожидает, что Apps Script вернет данные ученика в объекте `user`. После удаления лишних предметных колонок базовый лист `Лист1` лучше держать в таком виде:

| userId | username | lessons_remaining | coins | joined | lesson_link | schedule | subject | аватар |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |


- `userId` — ID, который вводится на странице входа.
- `username` — имя, которое показывается в профиле.
- `lessons_remaining` — вручную заполняемое количество оплаченных/оставшихся уроков; сайт показывает это число в профиле ученика. Можно назвать колонку по-русски `осталось уроков`, `оплаченные уроки`, `оплаченных уроков` или `остаток уроков`, Apps Script тоже ее найдет.
...
Из `Лист1` можно удалить старые колонки `progress`, `level_english` и `level_physics`: предметный прогресс и уровни берутся только из отдельного листа `Прогресс`. Apps Script читает колонки `Лист1` по заголовкам и не ломается при сдвиге столбцов влево после удаления.
