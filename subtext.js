const API_URL = "https://script.google.com/macros/s/AKfycbwuqVkx9CI58VemTx4SYd1miE4eD0vksT2jFS0HKfOAUtRsI0RUbv3vwv2ujgd1R3S7/exec";
let userId = "";
let username = "";
let currentCourse = "";
...
function renderCourseData() {
  ...
  if (materials) {
    ...
    const visibleGroups = selectedKey === "all" ? groups : groups.filter(group => group.key === selectedKey);
    const filteredGroups = visibleGroups
      .map(group => ({
        ...group,
        items: query ? group.items.filter(m => normalizeCourseName(m.title).includes(query)) : group.items,
      }))
      .filter(group => group.items.length);

    materials.classList.remove("lessons-grid");
    materials.innerHTML = filteredGroups.length
      ? filteredGroups.map(group => `<div class="materials-group">
          <h3 class="materials-group-title">${escapeHtml(group.label)}</h3>
          <div class="lessons-grid">${group.items.map(renderMaterialCard).join("")}</div>
        </div>`).join("")
      : `<p>${query ? "По этому запросу материалы не найдены." : "Материалы пока не добавлены."}</p>`;
  }
